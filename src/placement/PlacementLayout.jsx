// ─────────────────────────────────────────────────────────────
// Placement Management module shell — own sidebar + topbar.
// Sits at /placement/* (outside the role ERP chrome).
// ─────────────────────────────────────────────────────────────
import { useState } from 'react'
import { NavLink, Link, Outlet, useLocation } from 'react-router-dom'
import { motion } from 'framer-motion'
import { Icon } from '../components/ui.jsx'
import usePlacement, { selStudents, selOffers } from '../store/placementStore.js'

const NAV = [
  { label: 'Dashboard',       icon: 'LayoutDashboard', to: '/placement' },
  { label: 'Companies',       icon: 'Building2',       to: '/placement/companies',   badge: 'companies' },
  { label: 'Students',        icon: 'Users',           to: '/placement/students',    badge: 'registered' },
  { label: 'Eligibility',     icon: 'ShieldCheck',     to: '/placement/eligibility', badge: 'eligible' },
  { label: 'Placement Drives',icon: 'CalendarCheck',   to: '/placement/drives',      badge: 'drives' },
  { label: 'Assessments',     icon: 'FileText',        to: '/placement/assessments' },
  { label: 'Interviews',      icon: 'MessagesSquare',  to: '/placement/interviews',  badge: 'interviews', tone: 'warn' },
  { label: 'Offers',          icon: 'Gift',            to: '/placement/offers',      badge: 'pending', tone: 'warn' },
  { label: 'Reports',         icon: 'BarChart3',       to: '/placement/reports' },
  { label: 'Analytics',       icon: 'TrendingUp',      to: '/placement/analytics' },
  { label: 'Settings',        icon: 'Settings',        to: '/placement/settings' },
]
...
function SideNav({ open, onClose }) {
  ...
  const counts = {
    ...
    drives:     8,
    interviews: students.filter((s) => s.status === 'Interview').length,
  ...
  return (
    <aside
      ...
      style={{ background: 'linear-gradient(176deg, #4338CA 0%, #3730A3 55%, #312E81 100%)' }}
    >
      <Link to="/" onClick={onClose} className="focus-ring flex items-center gap-3 px-5 py-5">
        <div className="grid h-9 w-9 place-items-center rounded-lg bg-white shadow-sm">
          <Icon name="Briefcase" size={18} className="text-indigo-700" strokeWidth={2.5} />
        </div>
        <div className="min-w-0">
          <p className="text-[15px] font-bold leading-tight tracking-tight text-white">Placement</p>
          <p className="truncate text-2xs uppercase tracking-[0.12em] text-indigo-100/55">Training & Placement Cell</p>
        </div>
      </Link>
      ...
      <div className="px-3">
        <div className="flex items-center gap-2.5 rounded-lg bg-white/[0.08] px-3 py-2.5 ring-1 ring-inset ring-white/10">
          <div className="grid h-7 w-7 place-items-center rounded-md bg-white/10">
            <Icon name="Calendar" size={14} className="text-indigo-100" strokeWidth={2.3} />
          </div>
          <div className="leading-tight">
            <p className="text-xs font-bold text-white">Batch 2025</p>
            <p className="text-2xs text-indigo-100/55">Placement Season · Active</p>
          </div>
          ...
          <NavLink
            ...
            className={({ isActive }) =>
              `group relative flex items-center gap-2.5 overflow-hidden rounded-lg py-2 pl-4 pr-2.5 text-[13px] font-medium transition-all duration-200 ease-smooth ${
                isActive ? 'bg-white text-indigo-800 shadow-sm' : 'text-indigo-50/80 hover:bg-white/[0.10] hover:pl-[1.1rem] hover:text-white'
              }`
            }
          >
            {({ isActive }) => (
              <>
                {isActive && (
                  <motion.span layoutId="plc-nav-active"
                    className="absolute left-0 top-1/2 h-6 w-1 -translate-y-1/2 rounded-r-full bg-yellow-300" />
                )}
                <Icon name={item.icon} size={16} strokeWidth={2.2}
                  className={`shrink-0 transition-transform duration-200 ease-smooth group-hover:scale-110 ${
                    isActive ? 'text-indigo-700' : 'text-indigo-100/70 group-hover:text-white'
                  }`} />
                ...
                {item.badge && counts[item.badge] > 0 && (
                  <span className={`shrink-0 rounded-full px-1.5 py-0.5 text-[10px] font-bold leading-none transition-colors duration-200 ${
                    isActive
                      ? (item.tone === 'warn' ? 'bg-amber-100 text-amber-700' : 'bg-indigo-700/10 text-indigo-700')
                      : (item.tone === 'warn' ? 'bg-amber-400/20 text-amber-200' : 'bg-white/10 text-indigo-50/80 group-hover:bg-white/20')
                  }`}>
                    {counts[item.badge]}
                  </span>
                )}
      ...
      <div className="border-t border-white/[0.08] p-3">
        <Link to="/" onClick={onClose}
          className="flex items-center gap-2.5 rounded-lg px-3 py-2 text-[13px] font-semibold text-indigo-50/75 transition-colors duration-200 hover:bg-white/[0.08] hover:text-white">
...
export default function PlacementLayout() {
  ...
  const current = NAV.slice().reverse().find((n) =>
    n.to === '/placement' ? pathname === '/placement' : pathname.startsWith(n.to))
      ...
      <div className="lg:pl-[260px]">
        <header className="sticky top-0 z-20 flex items-center gap-3 border-b border-slate-200/80 bg-white/85 px-4 py-2.5 backdrop-blur-xl sm:px-6">
          ...
          <div className="hidden items-center gap-2 sm:flex">
            ...
            <span className="text-xs font-bold text-navy">{current?.label || 'Placement'}</span>
          </div>
          ...
          <div className="ml-auto flex items-center gap-2 md:ml-0">
            ...
            <div className="flex items-center gap-2.5 rounded-lg border border-slate-200 py-1 pl-1 pr-3 transition-colors hover:bg-slate-50">
              <span className="grid h-7 w-7 place-items-center rounded-md text-2xs font-bold text-white" style={{ background: 'linear-gradient(145deg,#4338CA,#312E81)' }}>TO</span>
              <div className="hidden leading-tight sm:block">
                <p className="text-xs font-semibold text-navy">Placement Officer</p>
                <p className="text-2xs text-slate-400">Training & Placement Cell</p>
              </div>
            </div>
          </div>
        </header>
