...
const NAV = [
  ...
  { label: 'Academic',          icon: 'GraduationCap',   to: '/analytics/academic'      },
  { label: 'Financial',         icon: 'IndianRupee',     to: '/analytics/financial'     },
  { label: 'Examinations',      icon: 'ClipboardList',   to: '/analytics/examinations'  },
  { label: 'Placements',        icon: 'Briefcase',       to: '/analytics/placements'    },
  { label: 'Students',          icon: 'Users',           to: '/analytics/students'      },
  { label: 'Infrastructure',    icon: 'Building2',       to: '/analytics/infrastructure'},
  { label: 'Reports',           icon: 'FileBarChart',    to: '/analytics/reports'       },
  { label: 'Settings',          icon: 'Settings',        to: '/analytics/settings'      },
]
    ...
    >
      <Link to="/" onClick={onClose} className="focus-ring flex items-center gap-3 border-b border-white/[0.08] px-5 py-4">
        <div className="grid h-10 w-10 place-items-center rounded-xl gold-gradient shadow-md flex-shrink-0">
          <Icon name="BarChart3" size={18} className="text-[#0C1540]" strokeWidth={2.5} />
        </div>
        <div className="min-w-0">
          <p className="text-[15px] font-bold leading-tight tracking-tight text-white">Analytics</p>
          <p className="text-2xs font-semibold uppercase tracking-[0.12em]" style={{ color: 'rgba(245,184,0,0.7)' }}>SCET ERP Module</p>
        </div>
      </Link>
      ...
      <div className="px-4 py-3">
        <div className="flex items-center gap-2.5 rounded-xl bg-white/[0.07] px-3 py-2.5 ring-1 ring-inset ring-white/10">
          <div className="grid h-7 w-7 place-items-center rounded-lg" style={{ background: 'rgba(245,184,0,0.2)' }}>
            <Icon name="TrendingUp" size={14} style={{ color: '#F5B800' }} strokeWidth={2.4} />
          </div>
          <div className="leading-tight">
            <p className="text-xs font-bold text-white">Institution Analytics</p>
            <p className="text-2xs text-white/40">AY 2024-25 · Live Data</p>
          </div>
      ...
      <nav className="nav-scroll mt-1 flex-1 space-y-0.5 overflow-y-auto px-3 pb-4">
        {NAV.map((item) => (
          <NavLink key={item.to} to={item.to} end={item.to === '/analytics'} onClick={onClose}
            className={({ isActive }) =>
          ...
          >
            {({ isActive }) => (
              <>
                {isActive && (
                  <motion.span layoutId="analytics-active"
                ...
                {isActive && <span className="h-1.5 w-1.5 rounded-full mr-1" style={{ background: '#F5B800' }} />}
              </>
            )}
          </NavLink>
        ))}
...
export default function AnalyticsLayout() {
  ...
  const current = NAV.slice().reverse().find((n) =>
    n.to === '/analytics' ? pathname === '/analytics' : pathname.startsWith(n.to))
      ...
      <div className="lg:pl-[268px]">
        <header className="sticky top-0 z-20 flex items-center gap-3 border-b border-[#E3E8F4] bg-white/90 px-4 py-2.5 backdrop-blur-xl sm:px-6">
          ...
          <div className="hidden items-center gap-2 sm:flex">
            ...
            <span className="rounded-md bg-[#EBF0FB] px-2 py-0.5 text-xs font-bold text-navy">{current?.label ?? 'Analytics'}</span>
          </div>
          <div className="ml-auto flex items-center gap-2.5">
            <span className="hidden items-center gap-1.5 rounded-lg border border-[#FDE68A] bg-[#FFFBEB] px-3 py-1.5 text-xs font-semibold text-amber-700 sm:flex">
              <span className="h-1.5 w-1.5 animate-pulse rounded-full bg-[#F5B800]" />
              Live Data · AY 2024-25
            </span>
            <div className="flex items-center gap-2.5 rounded-lg border border-[#E3E8F4] py-1 pl-1 pr-3 transition-colors hover:bg-slate-50">
              <span className="grid h-7 w-7 place-items-center rounded-md brand-gradient text-2xs font-bold text-white">SA</span>
              <div className="hidden leading-tight sm:block">
                <p className="text-xs font-semibold text-navy">System Admin</p>
                <p className="text-2xs text-slate-400">Analytics Access</p>
              </div>
            </div>
          </div>
        </header>

        <main className="mx-auto max-w-[1380px] px-4 py-7 sm:px-7">
          <Outlet />
        </main>
      </div>
    </div>
