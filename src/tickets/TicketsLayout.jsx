...
const NAV = [
  ...
  { label: 'All Tickets',   icon: 'Ticket',          to: '/tickets/all'            },
  { label: 'Raise Ticket',  icon: 'Plus',            to: '/tickets/raise'          },
  { label: 'Knowledge Base',icon: 'BookOpen',        to: '/tickets/knowledge-base' },
  { label: 'Agents',        icon: 'Users',           to: '/tickets/agents'         },
  { label: 'Reports',       icon: 'BarChart3',       to: '/tickets/reports'        },
  { label: 'Settings',      icon: 'Settings',        to: '/tickets/settings'       },
]
    ...
    >
      <Link to="/" onClick={onClose} className="focus-ring flex items-center gap-3 border-b border-white/[0.08] px-5 py-4">
        <div className="grid h-10 w-10 place-items-center rounded-xl gold-gradient shadow-md flex-shrink-0">
          <Icon name="Headset" size={18} className="text-[#0C1540]" strokeWidth={2.5} />
        </div>
        <div className="min-w-0">
          <p className="text-[15px] font-bold leading-tight tracking-tight text-white">Help Desk</p>
      ...
      <div className="px-4 py-3">
        <div className="flex items-center gap-2.5 rounded-xl bg-white/[0.07] px-3 py-2.5 ring-1 ring-inset ring-white/10">
          <div className="grid h-7 w-7 place-items-center rounded-lg bg-[#F5B800]/20">
            <Icon name="Headset" size={14} className="text-[#F5B800]" strokeWidth={2.4} />
          </div>
          <div className="leading-tight">
            <p className="text-xs font-bold text-white">IT Support Admin</p>
            <p className="text-2xs text-white/40">SCET Help Desk</p>
          </div>
      ...
      <nav className="nav-scroll mt-1 flex-1 space-y-0.5 overflow-y-auto px-3 pb-4">
        {NAV.map((item) => (
          <NavLink key={item.to} to={item.to} end={item.to === '/tickets'} onClick={onClose}
            className={({ isActive }) =>
          ...
          >
            {({ isActive }) => (
              <>
                {isActive && (
                  <motion.span layoutId="tickets-active"
...
export default function TicketsLayout() {
  ...
  const current = NAV.slice().reverse().find((n) =>
    n.to === '/tickets' ? pathname === '/tickets' : pathname.startsWith(n.to))
      ...
      <div className="lg:pl-[268px]">
        <header className="sticky top-0 z-20 flex items-center gap-3 border-b border-[#E3E8F4] bg-white/90 px-4 py-2.5 backdrop-blur-xl sm:px-6">
          ...
          <div className="hidden items-center gap-2 sm:flex">
            ...
            <span className="rounded-md bg-[#EBF0FB] px-2 py-0.5 text-xs font-bold text-navy">{current?.label ?? 'Help Desk'}</span>
          </div>
          <div className="relative ml-auto hidden max-w-xs flex-1 items-center md:flex">
            ...
            <input placeholder="Search tickets or KB articles…"
          ...
          <div className="ml-auto flex items-center gap-2 md:ml-0">
            <div className="flex items-center gap-2.5 rounded-lg border border-[#E3E8F4] py-1 pl-1 pr-3 transition-colors hover:bg-slate-50">
              <span className="grid h-7 w-7 place-items-center rounded-md brand-gradient text-2xs font-bold text-white">HD</span>
              <div className="hidden leading-tight sm:block">
                <p className="text-xs font-semibold text-navy">IT Support</p>
                <p className="text-2xs text-slate-400">Help Desk</p>
              </div>
            </div>
          </div>
        </header>
