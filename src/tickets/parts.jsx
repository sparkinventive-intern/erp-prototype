// Tickets / Help Desk module — shared presentational parts.
...
const PRIORITY_CONFIG = {
  Critical: { bg: 'bg-red-100',    text: 'text-red-800',    ring: 'ring-red-200',    dot: 'bg-red-600'    },
  High:     { bg: 'bg-orange-50',  text: 'text-orange-700', ring: 'ring-orange-200', dot: 'bg-orange-500' },
  Medium:   { bg: 'bg-[#FFFBEB]', text: 'text-amber-700',  ring: 'ring-[#FDE68A]',  dot: 'bg-[#F5B800]'  },
  Low:      { bg: 'bg-slate-50',   text: 'text-slate-600',  ring: 'ring-slate-200',  dot: 'bg-slate-400'  },
}

const STATUS_CONFIG = {
  Open:       { bg: 'bg-red-50',     text: 'text-red-700',     dot: 'bg-red-500',     ring: 'ring-red-200'     },
  ...
  Closed:     { bg: 'bg-slate-50',   text: 'text-slate-600',   dot: 'bg-slate-400',   ring: 'ring-slate-200'   },
}

export function PriorityBadge({ priority }) {
  const cfg = PRIORITY_CONFIG[priority] ?? PRIORITY_CONFIG.Low
  return (
    <span className={`inline-flex items-center gap-1.5 rounded-full px-2.5 py-0.5 text-xs font-bold ring-1 ring-inset ${cfg.bg} ${cfg.text} ${cfg.ring}`}>
