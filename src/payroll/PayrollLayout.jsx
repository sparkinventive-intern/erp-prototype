...
const NAV = [
  { label: 'Dashboard',         icon: 'LayoutDashboard', to: '/payroll'            },
  { label: 'Employees',         icon: 'Users',           to: '/payroll/employees'  },
  { label: 'Salary Structure',  icon: 'FileSpreadsheet', to: '/payroll/structure'  },
  { label: 'Payroll Processing',icon: 'Settings2',       to: '/payroll/process'    },
  { label: 'Attendance Sync',   icon: 'CalendarCheck',   to: '/payroll/attendance' },
  { label: 'Payslips',          icon: 'FileText',        to: '/payroll/payslips'   },
  { label: 'Deductions',        icon: 'Receipt',         to: '/payroll/deductions' },
  { label: 'Reports',           icon: 'BarChart3',       to: '/payroll/reports'    },
  { label: 'Analytics',         icon: 'LineChart',       to: '/payroll/analytics'  },
  { label: 'Settings',          icon: 'Settings',        to: '/payroll/settings'   },
]
    ...
    >
      ...
      <Link to="/" onClick={onClose} className="focus-ring flex items-center gap-3 border-b border-white/[0.08] px-5 py-4">
        <div className="grid h-10 w-10 place-items-center rounded-xl gold-gradient shadow-md flex-shrink-0">
          <Icon name="Wallet" size={18} className="text-[#0C1540]" strokeWidth={2.5} />
        </div>
        <div className="min-w-0">
          <p className="text-[15px] font-bold leading-tight tracking-tight text-white">Payroll</p>
      ...
      <div className="px-4 py-3">
        <div className="flex items-center gap-2.5 rounded-xl bg-white/[0.07] px-3 py-2.5 ring-1 ring-inset ring-white/10">
          <div className="grid h-7 w-7 place-items-center rounded-lg bg-[#F5B800]/20">
            <Icon name="IndianRupee" size={14} className="text-[#F5B800]" strokeWidth={2.4} />
          </div>
          <div className="leading-tight">
            <p className="text-xs font-bold text-white">HR & Payroll</p>
            <p className="text-2xs text-white/40">Finance Department</p>
          </div>
      ...
      <nav className="nav-scroll mt-1 flex-1 space-y-0.5 overflow-y-auto px-3 pb-4">
        {NAV.map((item) => (
          <NavLink key={item.to} to={item.to} end={item.to === '/payroll'} onClick={onClose}
            className={({ isActive }) =>
          ...
          >
            {({ isActive }) => (
              <>
                {isActive && (
                  <motion.span layoutId="pay-active"
...
export default function PayrollLayout() {
  ...
  const current = NAV.slice().reverse().find((n) =>
    n.to === '/payroll' ? pathname === '/payroll' : pathname.startsWith(n.to))
      ...
      <div className="lg:pl-[268px]">
        ...
        <header className="sticky top-0 z-20 flex items-center gap-3 border-b border-[#E3E8F4] bg-white/90 px-4 py-2.5 backdrop-blur-xl sm:px-6">
          ...
          <div className="hidden items-center gap-2 sm:flex">
            ...
            <span className="rounded-md bg-[#EBF0FB] px-2 py-0.5 text-xs font-bold text-navy">{current?.label ?? 'Payroll'}</span>
          </div>
          ...
          <div className="ml-auto flex items-center gap-2 md:ml-0">
            <div className="flex items-center gap-2.5 rounded-lg border border-[#E3E8F4] py-1 pl-1 pr-3 transition-colors hover:bg-slate-50">
              <span className="grid h-7 w-7 place-items-center rounded-md brand-gradient text-2xs font-bold text-white">HR</span>
              <div className="hidden leading-tight sm:block">
                <p className="text-xs font-semibold text-navy">HR Manager</p>
                <p className="text-2xs text-slate-400">Accounts & HR</p>
              </div>
            </div>
          </div>
        </header>
