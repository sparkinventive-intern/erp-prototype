...
const NAV = [
  { label: 'Dashboard',     icon: 'LayoutDashboard', to: '/library'             },
  { label: 'Catalog',       icon: 'BookOpen',        to: '/library/catalog'     },
  { label: 'Issue Books',   icon: 'ArrowUpRight',    to: '/library/issue'       },
  { label: 'Return Books',  icon: 'ArrowDownLeft',   to: '/library/return'      },
  { label: 'Reservations',  icon: 'Clock',           to: '/library/reservations'},
  { label: 'Members',       icon: 'Users',           to: '/library/members'     },
  { label: 'Fine Management',icon:'IndianRupee',     to: '/library/fines'       },
  { label: 'Digital Library',icon:'MonitorPlay',     to: '/library/digital'     },
  { label: 'Acquisition',   icon: 'ShoppingCart',    to: '/library/acquisition' },
  { label: 'Reports',       icon: 'BarChart3',       to: '/library/reports'     },
  { label: 'Analytics',     icon: 'LineChart',       to: '/library/analytics'   },
  { label: 'Settings',      icon: 'Settings',        to: '/library/settings'    },
]
    ...
    >
      <Link to="/" onClick={onClose} className="focus-ring flex items-center gap-3 border-b border-white/[0.08] px-5 py-4">
        <div className="grid h-10 w-10 place-items-center rounded-xl gold-gradient shadow-md flex-shrink-0">
          <Icon name="Library" size={18} className="text-[#0C1540]" strokeWidth={2.5} />
        </div>
        <div className="min-w-0">
          <p className="text-[15px] font-bold leading-tight tracking-tight text-white">Library</p>
      ...
      <div className="px-4 py-3">
        <div className="flex items-center gap-2.5 rounded-xl bg-white/[0.07] px-3 py-2.5 ring-1 ring-inset ring-white/10">
          <div className="grid h-7 w-7 place-items-center rounded-lg bg-[#F5B800]/20">
            <Icon name="BookMarked" size={14} className="text-[#F5B800]" strokeWidth={2.4} />
          </div>
          <div className="leading-tight">
            <p className="text-xs font-bold text-white">Chief Librarian</p>
            <p className="text-2xs text-white/40">Central Library</p>
          </div>
      ...
      <nav className="nav-scroll mt-1 flex-1 space-y-0.5 overflow-y-auto px-3 pb-4">
        {NAV.map((item) => (
          <NavLink key={item.to} to={item.to} end={item.to === '/library'} onClick={onClose}
            className={({ isActive }) =>
          ...
          >
            {({ isActive }) => (
              <>
                {isActive && (
                  <motion.span layoutId="lib-active"
...
export default function LibraryLayout() {
  ...
  const current = NAV.slice().reverse().find((n) =>
    n.to === '/library' ? pathname === '/library' : pathname.startsWith(n.to))
      ...
      <div className="lg:pl-[268px]">
        <header className="sticky top-0 z-20 flex items-center gap-3 border-b border-[#E3E8F4] bg-white/90 px-4 py-2.5 backdrop-blur-xl sm:px-6">
          ...
          <div className="hidden items-center gap-2 sm:flex">
            ...
            <span className="rounded-md bg-[#EBF0FB] px-2 py-0.5 text-xs font-bold text-navy">{current?.label ?? 'Library'}</span>
          </div>
          ...
          <div className="ml-auto flex items-center gap-2 md:ml-0">
            <div className="flex items-center gap-2.5 rounded-lg border border-[#E3E8F4] py-1 pl-1 pr-3 transition-colors hover:bg-slate-50">
              <span className="grid h-7 w-7 place-items-center rounded-md brand-gradient text-2xs font-bold text-white">LB</span>
              <div className="hidden leading-tight sm:block">
                <p className="text-xs font-semibold text-navy">Chief Librarian</p>
                <p className="text-2xs text-slate-400">Central Library</p>
              </div>
            </div>
          </div>
        </header>
