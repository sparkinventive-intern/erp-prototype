// ─────────────────────────────────────────────────────────────
// Result Management module shell — dedicated sidebar + topbar.
// Sits at /results/* (outside the role ERP chrome).
// ─────────────────────────────────────────────────────────────
...
import { isPass } from '../data/resultData.js'

const NAV = [
  { label: 'Dashboard', icon: 'LayoutDashboard', to: '/results' },
  { label: 'Result Processing', icon: 'GitBranch', to: '/results/processing' },
  { label: 'Student Results', icon: 'Users', to: '/results/students', badge: 'total' },
  { label: 'Grade Rules', icon: 'ListChecks', to: '/results/grades' },
  { label: 'GPA Calculator', icon: 'Calculator', to: '/results/gpa-calculator' },
  { label: 'Rank Lists', icon: 'Trophy', to: '/results/ranks' },
  { label: 'Backlog Management', icon: 'AlertTriangle', to: '/results/backlog', badge: 'backlog', tone: 'warn' },
  { label: 'Revaluation', icon: 'FileSearch', to: '/results/revaluation' },
  { label: 'Transcripts', icon: 'FileText', to: '/results/transcripts' },
  { label: 'Result Publication', icon: 'Megaphone', to: '/results/publication' },
  { label: 'Analytics', icon: 'BarChart3', to: '/results/analytics' },
  { label: 'Settings', icon: 'Settings', to: '/results/settings' },
]

function SideNav({ open, onClose }) {
  const results = useResults(selResults)
  const counts = { total: results.length, backlog: results.filter((r) => !isPass(r.subjects)).length }
  return (
    <aside
      className={`fixed z-40 flex h-full w-[260px] flex-col transition-transform duration-300 ease-smooth lg:translate-x-0 ${
        open ? 'translate-x-0' : '-translate-x-full'
      }`}
      style={{ background: 'linear-gradient(176deg, #047857 0%, #065F46 55%, #064E3B 100%)' }}
    >
      <Link to="/" onClick={onClose} className="focus-ring flex items-center gap-3 px-5 py-5">
        <div className="grid h-9 w-9 place-items-center rounded-lg bg-white shadow-sm">
          <Icon name="Award" size={18} className="text-emerald-700" strokeWidth={2.4} />
        </div>
        <div className="min-w-0">
          <p className="text-[15px] font-bold leading-tight tracking-tight text-white">Results</p>
          <p className="truncate text-2xs uppercase tracking-[0.12em] text-emerald-100/55">Spark ERP Module</p>
        </div>
      </Link>
      ...
      <div className="px-3">
        <div className="flex items-center gap-2.5 rounded-lg bg-white/[0.08] px-3 py-2.5 ring-1 ring-inset ring-white/10">
          <div className="grid h-7 w-7 place-items-center rounded-md bg-white/10">
            <Icon name="GraduationCap" size={14} className="text-emerald-100" strokeWidth={2.3} />
          </div>
          <div className="leading-tight">
            <p className="text-xs font-bold text-white">Semester VI</p>
            <p className="text-2xs text-emerald-100/55">2026 · Processing</p>
          </div>
          ...
          <NavLink
            ...
            className={({ isActive }) =>
              `group relative flex items-center gap-2.5 overflow-hidden rounded-lg py-2 pl-4 pr-2.5 text-[13px] font-medium transition-all duration-200 ease-smooth ${
                isActive ? 'bg-white text-emerald-800 shadow-sm' : 'text-emerald-50/80 hover:bg-white/[0.10] hover:pl-[1.1rem] hover:text-white'
              }`
            }
          >
            {({ isActive }) => (
              <>
                {isActive && (
                  <motion.span layoutId="result-nav-active"
                ...
                {item.badge && counts[item.badge] > 0 && (
                  <span className={`shrink-0 rounded-full px-1.5 py-0.5 text-[10px] font-bold leading-none transition-colors duration-200 ${
                    isActive
                      ? (item.tone === 'warn' ? 'bg-amber-100 text-amber-700' : 'bg-emerald-700/10 text-emerald-700')
                      : (item.tone === 'warn' ? 'bg-amber-400/20 text-amber-100' : 'bg-white/10 text-emerald-50/80 group-hover:bg-white/20')
                  }`}>
                    {counts[item.badge]}
                  </span>
                )}
      ...
      <div className="border-t border-white/[0.08] p-3">
        <Link to="/" onClick={onClose}
          className="flex items-center gap-2.5 rounded-lg px-3 py-2 text-[13px] font-semibold text-emerald-50/75 transition-colors duration-200 hover:bg-white/[0.08] hover:text-white">
...
export default function ResultLayout() {
  ...
  const current = NAV.slice().reverse().find((n) =>
    n.to === '/results' ? pathname === '/results' : pathname.startsWith(n.to))
      ...
      <div className="lg:pl-[260px]">
        <header className="sticky top-0 z-20 flex items-center gap-3 border-b border-slate-200/80 bg-white/85 px-4 py-2.5 backdrop-blur-xl sm:px-6">
          ...
          <div className="hidden items-center gap-2 sm:flex">
            ...
            <span className="text-xs font-bold text-navy">{current?.label || 'Results'}</span>
          </div>
          ...
          <div className="ml-auto flex items-center gap-2 md:ml-0">
            ...
            <div className="flex items-center gap-2.5 rounded-lg border border-slate-200 py-1 pl-1 pr-3 transition-colors hover:bg-slate-50">
              <span className="grid h-7 w-7 place-items-center rounded-md text-2xs font-bold text-white" style={{ background: 'linear-gradient(145deg,#047857,#064E3B)' }}>RA</span>
              <div className="hidden leading-tight sm:block">
                <p className="text-xs font-semibold text-navy">Result Analyst</p>
